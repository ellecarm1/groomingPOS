    ...
    <main className="invoice-print-page min-h-screen bg-[#f5f3ed] px-5 py-8 text-[#1e352c] sm:px-8 lg:px-12 print:bg-white print:p-0">
      <div className="mx-auto max-w-[780px]">
        <div className="print-controls mb-6 flex items-center justify-between print:hidden">
          ...
          <button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-xl bg-[#234438] px-4 py-2.5 text-xs font-extrabold text-white transition hover:bg-[#315847]"><Printer className="h-4 w-4" /> Print / save PDF</button>
        </div>

        <InvoicePreview services={services} selected={selected} booking={booking} />

        <p className="print-controls mt-5 text-center text-[11px] font-semibold leading-5 text-[#9aa49a] print:hidden">Windows tip: choose <span className="font-extrabold text-[#6f7e70]">Microsoft Print to PDF</span> in the browser print dialog to save a PDF copy.</p>
      </div>
    </main>
  );
}
